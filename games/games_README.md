...
so that `opponent` is unambiguous. Runtime evaluation supports both
`no_pieces_left` and `no_moves_left`; the latter uses the legal moves generated
for the next player after turn rotation.
